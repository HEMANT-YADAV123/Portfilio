  ...
  const handleSend = async (e) => {
    ...
    if (Username === "") {
      ...
      try {
        ...
        const response = await fetch("https://xenoai-backend.onrender.com/send-email", {
          method: "POST",
          headers: {
      ...
      <form className="w-full flex flex-col gap-6 lg:py-5 py-2">
        ...
        <div className="flex flex-col gap-4">
          ...
          <input
            ...
            value={Subject}
            className={`${
              errMsg === "Please Enter Subject" && "outline-designColor"
            } contactInput`}
            type="text"
          />
        </div>
        ...
        <div className="w-full">
          <button onClick={handleSend} className="contactButton">
            Send Message
          </button>
        </div>
      </form>
